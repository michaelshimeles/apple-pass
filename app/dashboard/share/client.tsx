...
export function SharePreviewClient({ pass, userPasses }: { pass: ApplePass; userPasses: PassItem[] }) {
  const [passName, setPassName] = useState(pass?.name || "");
  const [selectedPass, setSelectedPass] = useState<string | undefined>(pass?.pass_share_id);
  ...
  return (
    <div className="flex flex-col h-screen p-6 w-full">
      <div className="flex justify-between items-center mb-6">
        ...
        {userPasses?.length > 0 && (
          <Select defaultValue={pass?.pass_share_id} onValueChange={handlePassChange}>
      ...
      <div className="flex justify-start items-start gap-[5rem] w-full">
        ...
        <div className="w-full max-w-[400px] animate-fadeInUp" style={{ animationDelay: '0.3s' }}>
          <Card className="shadow-md border-slate-200 rounded">
            <CardHeader>
              <CardTitle className="text-xl text-slate-800">Customize Your Pass Page</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label className="text-sm font-medium text-slate-700">Pass Title</Label>
                <Input 
                ...
                />
                <p className="text-xs text-slate-500 mt-1">This will be displayed when sharing your pass.</p>
              </div>
            </CardContent>
            <CardFooter>
              <Button
